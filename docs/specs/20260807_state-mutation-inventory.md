...
| `state/task_results/{task_id}.md` | runner pending executor write `core/tasks/pending_executor.py:402-410`; inboxはread-only `core/_anima_inbox.py:234-260`; server housekeeping delete `core/memory/housekeeping.py:1457-1484` | task IDごとのplain overwrite、retention deleteと共通lockなし | task runner単一writer + server retention cleanup。実行中task IDは削除対象外にfence | root task APIがwrite/deleteを一元化 |
