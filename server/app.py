...
async def _startup_animas_background(app: FastAPI, *, suppress_errors: bool = True) -> None:
    ...
    try:
        ...
        #     if missing:
        #         logger.warning("Slack tokens missing for: %s", ", ".join(missing))
        # except Exception:
        ...
        # ── Avatar upload to XSERVER ──────────────────────────────────────
        try:
            from server.avatar_upload import upload_all_avatars
...
async def lifespan(app: FastAPI):
    ...
    if app.state.setup_complete:
        ...
        # ── Slack channel → board periodic resync (disabled – Discord migration) ──
        ...
        # Vector worker start + child_env_urls are handled inside
        # _run_startup_initialization → _prepare_startup_vector_worker.
    ...
    if app.state.setup_complete:
        ...
        # Slack shutdown disabled – Discord migration
