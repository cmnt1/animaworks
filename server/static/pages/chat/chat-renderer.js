...
export function createChatRenderer(ctx) {
  ...
  function renderChat(scrollToBottom = true) {
    ...
    if (state.meetingMode && !state.meetingRoom) {
      const emptyLabel = t("meeting.select_session");
      messagesEl.innerHTML = `<div class="chat-empty">${emptyLabel !== "meeting.select_session" ? emptyLabel : "ミーティングを選択するか、新しく開始してください"}</div>`;
      return;
    }
