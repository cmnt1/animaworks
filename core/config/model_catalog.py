...
def _build_static_model_catalog(config: Any) -> list[dict[str, str]]:
    ...
    for item in KNOWN_MODELS:
        ...
        seen.add(model)
        if provider == "anthropic" and credential is not None and credential.api_key and model.startswith("claude-"):
            api_model = f"anthropic/{model}"
            models.append({"id": api_model, "label": model, "credential": provider, "mode": "A"})
            seen.add(api_model)
...
def available_model_id_set(config: Any = None) -> set[str]:
    ...
    if config is None:
        config = load_config()
    return discovered_model_ids(config=config) | {entry["id"] for entry in _build_static_model_catalog(config)}
