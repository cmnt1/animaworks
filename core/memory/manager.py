...
class MemoryManager:
    ...
    def _read_raw(self, path: Path) -> str:
        """Read *path* through the host deny check, leaving frontmatter intact."""
        ...
        try:
            ...
            logger.warning("Invalid UTF-8 in %s; reading with replacement characters", resolved, exc_info=True)
            try:
                return resolved.read_text(encoding="utf-8", errors="replace")
            except OSError:
                logger.warning("Failed to read %s after UTF-8 fallback", resolved, exc_info=True)
                return ""
        except OSError:
    ...
    def read_resolutions(self, days: int = 7) -> list[dict[str, str]]:
        ...
        content = self._read_raw(get_shared_dir() / "resolutions.jsonl")
        if not content:
            return []
