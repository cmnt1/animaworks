...
STRINGS: dict[str, dict[str, str]] = {
    ...
    "schema.delegate_task.desc": {
        "ja": (
            ...
            "【軽量/中量モデル配下の粒度ルール】部下には1タスク=1具体フェーズで委譲する。"
            "調査、DB修正、sync/deploy、公開検証、報告を1枚に詰め込まず、必要なら上位Animaが分解して順に投げる。"
            "allow_multistageは軽量/中量モデル配下では無視される。"
        ...
        "en": (
            ...
            "GRANULARITY RULE for lightweight/medium subordinate models: delegate one concrete phase per task. "
            "Do not pack investigation, DB fix, sync/deploy, public verification, and reporting into one card; the supervisor must split and sequence them. "
            "allow_multistage is ignored for lightweight/medium subordinate models. "
        ...
        "ko": (
            ...
            "【경량/중간 모델 부하의 입도 규칙】부하에게는 1태스크=1구체 단계로 위임한다. "
            "조사, DB 수정, sync/deploy, 공개 검증, 보고를 한 카드에 넣지 말고, 필요하면 상위 Anima가 분해하여 순서대로 위임한다. "
            "allow_multistage는 경량/중간 모델 부하에게는 무시된다. "
