    ...
    converter._append_audit(
        {
            ...
            "procedure_path": source_path.relative_to(converter._anima_dir).as_posix(),
            "skill_name": final_skill_name,
            "target_path": skill_md.relative_to(converter._anima_dir).as_posix(),
            "scan_verdict": scan_result.verdict.value,
        }
    )
    return SkillPromotionResult(
        ...
        skill_name=final_skill_name,
        procedure_path=source_path.relative_to(converter._anima_dir).as_posix(),
        active_path=skill_md.relative_to(converter._anima_dir).as_posix(),
        scan_verdict=scan_result.verdict.value,
        requires_human_approval=False,
        message=t(
            "skill_auto.created",
            skill_name=final_skill_name,
            path=skill_md.relative_to(converter._anima_dir).as_posix(),
        ),
        findings=[f.model_dump(mode="json") for f in scan_result.findings],
        size_violations=scan_result.size_violations,
    )
