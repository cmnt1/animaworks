...
def _split_segment(segment: str) -> list[str]:
    """Split a shell segment without consuming Windows path separators."""
    argv = shlex.split(segment, posix=os.name != "nt")
    if os.name == "nt":
        return [arg[1:-1] if len(arg) >= 2 and arg[0] == arg[-1] and arg[0] in {'"', "'"} else arg for arg in argv]
    return argv
