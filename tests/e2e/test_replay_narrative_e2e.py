...
class TestReplayNarrativeBand:
    ...
    def build_narrative_state(self, events: list[dict], current_ms: int) -> dict:
        visible = [evt for evt in events if evt.get("kind") and int(evt.get("importance") or 0) >= 2]
        current_event = None
        current_index = -1
        ...
        visible_events_in_last_hour = sum(1 for evt in visible if hour_before <= self.event_time_ms(evt) <= current_ms)
        return {
