...
async def test_mode_a_preserves_query_matched_skill_under_default_budget(make_agent_core, monkeypatch):
    agent = make_agent_core(name="a-ranked-skills", model="ollama/gemma3:27b", execution_mode="autonomous")
    ...
    with patch_litellm(make_litellm_response(content="Synthetic response.")) as mock_call:
        await agent.run_cycle("Please verify quasar release manifest.")

    messages = mock_call.await_args.kwargs["messages"]
    captured = [message["content"] for message in messages if message.get("role") == "system"]
    assert captured
