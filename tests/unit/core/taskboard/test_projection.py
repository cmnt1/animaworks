...
def test_missing_queue_metadata_is_archived_even_when_requested(tmp_path: Path) -> None:
    store = TaskBoardStore(tmp_path / "taskboard.sqlite3")
    store.upsert_metadata(
        anima_name="sakura",
        task_id="missing",
    ...
    projected = project_anima(anima_dir, store, include_missing=True, include_archived=True)
    assert len(projected) == 1
