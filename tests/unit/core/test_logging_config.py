...
def test_replace_current_link_creates_and_replaces(tmp_path):
    ...
    assert (link.readlink().name if link.is_symlink() else link.read_text(encoding="utf-8")) == "a.log"

    # Replacing an existing link must succeed without an unlink window.
    _replace_current_link(link, "b.log")
    assert (link.readlink().name if link.is_symlink() else link.read_text(encoding="utf-8")) == "b.log"
...
def test_replace_current_link_concurrent_calls(tmp_path):
    ...
    assert (link.readlink().name if link.is_symlink() else link.read_text(encoding="utf-8")) == "x.log"
