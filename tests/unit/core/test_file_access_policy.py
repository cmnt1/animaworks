...
def test_internal_cache_matcher_only_protects_runtime_credentials(tmp_path: Path) -> None:
    ...
    assert find_internal_cache_root(anima_dir / ".codex_home" / "auth.json", anima_dir) is not None
    assert all(find_internal_cache_root(path, anima_dir) is None for path in accessible)
...
def test_internal_cache_matcher_blocks_symlinks_in_both_directions(tmp_path: Path) -> None:
    ...
    allowed.mkdir()
    external.mkdir()
    cache = codex_home / "cache.txt"
    cache.write_text("secret", encoding="utf-8")
    (allowed / "into-cache").symlink_to(cache)
    (codex_home / "out-of-cache").symlink_to(external / "public.txt")

    assert find_internal_cache_root(allowed / "into-cache", anima_dir) is not None
    assert find_internal_cache_root(codex_home / "out-of-cache", anima_dir) is not None


def test_shell_internal_denies_only_include_runtime_credentials(tmp_path: Path) -> None:
    anima_dir = tmp_path / "anima"
    anima_dir.mkdir()

    denied = shell_internal_deny_paths(anima_dir)

    assert denied == ((anima_dir / ".codex_home").resolve(),)
