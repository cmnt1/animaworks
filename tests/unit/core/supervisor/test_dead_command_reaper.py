...
class TestOrphanSandbox:
    ...
    def test_sessions_are_deduplicated(self, fake_now, fake_data_dir) -> None:
        ...
        with patch.object(reaper.os, "getsid", side_effect=lambda pid: 99, create=True):
...
class TestKillExecution:
    def test_orphan_sandbox_kills_session_and_descendants(self, fake_now, fake_data_dir) -> None:
        ...
        with (
            patch.object(reaper.os, "getsid", return_value=999, create=True),
            patch.object(reaper.os, "killpg", create=True) as killpg,
        ...
        killpg.assert_called_once_with(999, reaper._KILL_SIGNAL)
    ...
    def test_kill_targets_logs_fixed_line_and_counts(self, fake_now, fake_data_dir) -> None:
        ...
        logs: list[str] = []
        with (
            patch.object(reaper.os, "getsid", return_value=401, create=True),
            patch.object(reaper.os, "killpg", create=True),
            patch.object(reaper.psutil, "Process") as ps_proc,
            patch.object(reaper.os, "kill"),
        ):
            ps_proc.return_value.children.return_value = []
            count = reaper.kill_targets(
                [target, {"pid": 402, "age": 1, "kind": "broad-search", "cmd": "x"}], log=logs.append
            )
        assert count == 2
        assert len(logs) == 2
    ...
    def test_kill_target_missing_process_is_harmless(self, fake_now, fake_data_dir) -> None:
        ...
        with (
            patch.object(reaper.os, "getsid", side_effect=ProcessLookupError, create=True),
            patch.object(reaper.psutil, "Process") as ps_proc,
