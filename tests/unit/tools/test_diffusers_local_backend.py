...
from core.integrations.image_gen import ImageGenPipeline, dispatch
...
def test_pipeline_uses_local_diffusers_for_fullbody(tmp_path: Path) -> None:
    ...
    with patch("core.integrations.image_gen.LocalDiffusersClient") as mock_cls:
...
def test_dispatch_generate_fullbody_uses_local_diffusers(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch("core.integrations.image_gen.LocalDiffusersClient") as mock_cls,
    ):
        mock_client = MagicMock()
