...
- **Eligible tool names** (`is_eligible`) merge these **three layers** (later wins). Keys are matched as dictionary keys (you may have both Mode A schema names like `generate_3d_model` and Mode S submit-style `image_gen:3d`):
  ...
  2. Via `BackgroundTaskManager.from_profiles`, subcommands with `background_eligible: true` from each module’s `EXECUTION_PROFILE` (`core.integrations._base.get_eligible_tools_from_profiles`). Keys are `"{tool_name}:{subcmd}"`; seconds come from `expected_seconds` (default 60 if unset).
