...
- MUST: `status: ok` の `Current Pre-Observed Heartbeat Snapshot` があれば、それを使いツールを重ねて呼ばない。事前観測がなければ、フォールバックとして Observe の最初に `heartbeat_observe_snapshot` を呼ぶ。
...
- MUST: 事前観測と直接呼出しのどちらからも `status: ok` を得られない場合に限り、同じ blocked 経路を繰り返さず、ブロッカーとして `state/current_state.md` に記録するか、必要に応じて報告する。
