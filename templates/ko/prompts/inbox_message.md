...
- **[MUST] 수행이 필요한 작업을 파악했다면, 반드시 이후에 남는 형태로 만드세요. 답변만 하고 잊어버려서는 안 됩니다. Inbox는 단명 세션이므로 여기서 시작한 다단계 작업은 세션 종료와 함께 사라집니다.**
  ...
  - 다단계·장시간 작업을 직접 하기 → **이 세션에서 실행하려 하지 마세요** (사라집니다). `write_memory_file`로 `state/pending.md`(백로그)에 작업 내용과 다음 액션을 추가하고 `state/current_state.md`에도 반영하세요. 다음 하트비트가 계획·실행합니다. 답변에는 착수 예정(요점과 대략적인 절차)을 적으세요
- 답변은 간결하게 (장문 불필요)
