...
프로젝트 관리의 단일 기준은 **Obsidian Vault**입니다 (MUST). Notion은 기본으로 사용하지 않습니다.

- 프로젝트 계획, 진행 상황, 산출물, 확인 대기, 결정 사항은 `E:\OneDriveBiz\Obsidian\` 아래의 Markdown에 읽고 씁니다
- 산출물과 보고서는 `common_skills/obsidian-product/SKILL.md` 규약에 따라 `E:\OneDriveBiz\Obsidian\_products\`에 기록합니다
- Anima 내부의 `task_queue.jsonl`과 `current_state.md`는 실행 관리용이며, 사람이 참조하는 프로젝트 장부를 대체하지 않습니다
- Notion 읽기/쓰기, Notion DB를 진행 관리 기준으로 사용하는 것, Notion에 산출물을 제출하는 것은 하지 않습니다
