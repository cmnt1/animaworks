...
description: >-
  ...
  承認済みシートを土台に「キャラだけ差し替える」方式と、その検証方法。
  Use when: animaのドット絵アセットを新規作成・差し替えするとき、pixel workspaceの見た目を直すとき。
tags: [image, pixel-art, codex, workspace]
---
...
承認済みシートを行ごとに切り出し、**POINTで400%拡大し、透明部分をマゼンタで埋めた
...
384×96のまま渡すと**モデルが顔を大きく描き直す**。必ず400%版を渡す。
